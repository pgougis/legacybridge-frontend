...
export default function AdminCall() {
  ...
  useEffect(() => {
    sourcesApi.getAll().then(s => {   // admin voit toutes les sources
      setSources(s)
  ...
  useEffect(() => {
    ...
    setOpLoading(true)
    sourcesApi.getOperations(sourceId)   // toutes les opérations WSDL, pas filtrées par plan
      .then(ops => {
        setOperations(ops)
  ...
  async function handleCall() {
    ...
    try {
      let parsed: unknown = {}
      try { parsed = JSON.parse(body) } catch { /* send as-is */ }
  ...
  return (
    <div className="page">
      <div className="page-hd">
        <div><h1>Call Legacy</h1><p>Dispatch a method call to a legacy system</p></div>
      </div>
